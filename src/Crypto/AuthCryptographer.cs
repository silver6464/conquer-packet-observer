namespace ConquerPoc.Cryptography
{
    public class AuthCryptography 
    {
        ...
        /// <summary>
        /// Apply the cipher in REVERSE order. The server's Encrypt/Decrypt above is
        /// non-involutory: applying it twice does NOT yield the original. The Conquer
        /// client's matching "encrypt-for-send" is this inverse routine. We need it
        /// in the proxy: when forwarding plaintext OUT to the real server, we have
        /// to encode it the way the client would, so the server's Decrypt restores
        /// plaintext. Similarly, when forwarding plaintext IN to the real client,
        /// we encode it the way the server would, so the client's Decrypt restores
        /// plaintext.
        /// </summary>
        public void EncryptInverse(byte[] In, byte[] Out, int Length, bool useOutCounter)
        {
            lock (this)
            {
                for (int i = 0; i < Length; i++)
                {
                    ushort c = useOutCounter ? this.OutCounter : this.InCounter;
                    byte o = (byte)(Key1[c & 0xff] ^ In[i]);
                    o = (byte)(Key2[c >> 8] ^ o);
                    o = (byte)((o << 4) | (o >> 4));
                    o = (byte)(o ^ 0xab);
                    Out[i] = o;
                    if (useOutCounter) this.OutCounter = (ushort)(this.OutCounter + 1);
                    else this.InCounter = (ushort)(this.InCounter + 1);
                }
            }
        }
