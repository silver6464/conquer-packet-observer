namespace ConquerPoc.Cryptography
{
    ...
    {
        private byte[] _key;
        private readonly BlowfishCfb64 _encrypt = new BlowfishCfb64();
        private readonly BlowfishCfb64 _decrypt = new BlowfishCfb64();
        ...
        {
            _key = k;
            _encrypt.Init(_key, _encrypt.Iv);
            _decrypt.Init(_key, _decrypt.Iv);
        }
    ...
    /// <summary>
    /// OpenSSL BF_cfb64_encrypt-compatible Blowfish CFB-64 byte-streaming mode,
    /// using an embedded Blowfish-ECB engine that accepts arbitrary key lengths
    /// (matches OpenSSL BF_set_key — BouncyCastle's BlowfishEngine caps at 56 bytes).
    /// </summary>
    internal sealed class BlowfishCfb64
    {
        private readonly BlowfishEcb _engine = new BlowfishEcb();
        ...
        public byte[] Iv
        {
            get { return (byte[])_feedback.Clone(); }
        }
        ...
        public void ProcessBytes(byte[] data, bool encrypting)
        {
            for (int i = 0; i < data.Length; i++)
            {
                if (_idx == 0)
                {
                    _engine.EncryptBlock(_feedback, 0, _keystream, 0);
                }

                byte inByte = data[i];
                byte outByte = (byte)(inByte ^ _keystream[_idx]);
                _feedback[_idx] = encrypting ? outByte : inByte;
                data[i] = outByte;
                _idx = (_idx + 1) & 7;
            }
        }
    ...
    /// <summary>
    /// Minimal Blowfish ECB engine matching OpenSSL BF_set_key / BF_encrypt semantics.
    /// Accepts arbitrary key lengths (1..72 bytes); longer keys wrap cyclically in the P-array XOR.
    /// </summary>
    internal sealed class BlowfishEcb
    {
        ...
        {
            System.Array.Copy(KP, _p, 18);
            System.Array.Copy(KS0, _s0, 256);
            System.Array.Copy(KS1, _s1, 256);
            System.Array.Copy(KS2, _s2, 256);
            System.Array.Copy(KS3, _s3, 256);
            ...
            {
                EncryptPair(ref l, ref r);
                _p[i] = l;
                _p[i + 1] = r;
            }
            for (int i = 0; i < 256; i += 2) { EncryptPair(ref l, ref r); _s0[i] = l; _s0[i + 1] = r; }
        ...
        {
            uint l = ((uint)inBuf[inOff] << 24) | ((uint)inBuf[inOff + 1] << 16) | ((uint)inBuf[inOff + 2] << 8) | inBuf[inOff + 3];
            uint r = ((uint)inBuf[inOff + 4] << 24) | ((uint)inBuf[inOff + 5] << 16) | ((uint)inBuf[inOff + 6] << 8) | inBuf[inOff + 7];

            EncryptPair(ref l, ref r);

            outBuf[outOff] = (byte)(l >> 24);
            outBuf[outOff + 1] = (byte)(l >> 16);
            outBuf[outOff + 2] = (byte)(l >> 8);
            outBuf[outOff + 3] = (byte)l;
            outBuf[outOff + 4] = (byte)(r >> 24);
            outBuf[outOff + 5] = (byte)(r >> 16);
            outBuf[outOff + 6] = (byte)(r >> 8);
            outBuf[outOff + 7] = (byte)r;
        }
        ...
        {
            return ((_s0[(byte)(x >> 24)] + _s1[(byte)(x >> 16)]) ^ _s2[(byte)(x >> 8)]) + _s3[(byte)x];
        }

        // Blowfish initial P-array: digits of pi (fractional part, hex).
        ...
        // S-boxes — also derived from pi. Same as OpenSSL bf_init.c.
